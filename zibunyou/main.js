...
const ROWS = 22;
const BLOCK_SIZE = 20;
// --- ぷよテト風DAS/ARR/ロックディレイ ---
const DAS = 100;   // ms ぷよテト初動
const ARR = 17;    // ms ぷよテトは16.67ms(1f)なのでほぼ1マス/フレーム
const LOCK_DELAY_GROUND = 500; // ms 設置時ロックディレイ
const LOCK_DELAY_MOVE = 500;   // ms 動かした直後の猶予も同様
let lockDelay = 0;
...
function rotate(dir) {
    ...
    // Oミノは回転しても見た目変わらないので特別扱い
    if (current === 'O') {
        pos.r = newR;
        return;
    }
    const kickType = (current === 'I') ? 'I' : 'normal';
    const kickTable = SRS_KICK_TABLE[kickType];
...
function checkLanding() {
    if (!isValid(pos.x, pos.y + 1, pos.r)) {
        ...
        if (lockDelay >= LOCK_DELAY_GROUND) {
            place();
            lockActive = false;
        }
    } else {
...
function drawHoldOrNext(ctx, type, offsetX, offsetY, scale=1, bgClear=true) {
    if(bgClear) ctx.clearRect(0,0,ctx.canvas.width,ctx.canvas.height);
    if (!type) return;
    ...
    // 中央寄せ・上下左右に1マス（または0.5マス）空ける
    let boxW = maxX-minX+1;
    let boxH = maxY-minY+1;
    let size = 16 * scale;
    let pad = (ctx.canvas.width/size-boxW)/2; // 左右パディング
    let vpad = (ctx.canvas.height/size-boxH)/2; // 上下パディング
    for (let [dx, dy] of shape) {
        let nx = dx-minX+pad+offsetX;
        let ny = dy-minY+vpad+offsetY;
        drawBlock(ctx, nx, ny, type, size);
    }
}
function draw() {
    // フィールド
    ...
    // HOLD欄（左上寄せ+中央寄せ上下左右1マス空き）
    drawHoldOrNext(holdCtx, hold, 0, 0, 1, true);
    // NEXT欄（各ミノ上下左右1マス空きで縦に積む: 4個表示）
    nextCtx.clearRect(0,0,nextCanvas.width,nextCanvas.height);
    for (let i=0; i<4; i++) {
        const nextType = queue[i];
        if (!nextType) continue;
        // 1マス空けて縦配置
        drawHoldOrNext(nextCtx, nextType, 0, i*2, 1, false);
    }
    // スコアなど
...
function update(now) {
    ...
    lastTime = now;
    dropCounter += (softDrop ? 4/60 : 1/60) * delta;
    if (dropCounter >= 1) {
        if (!tryMove(0, 1)) {
            checkLanding();
        } else {
